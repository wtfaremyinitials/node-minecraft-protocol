...
Client.prototype.setSocket = function(socket) {
  ...
  self.socket.on('data', function(data) {
    ...
    var whatever = function(parsed) {
      if (parsed) {
        if (parsed.error) {
          self.emit('error', parsed.error);
    ...
    attempt = function() {
      if(self.compressionThreshold == -2)
        whatever(parsePacket(incomingBuffer, self.state, self.isServer, self.packetsToParse));
      else
        parseNewStylePacket(incomingBuffer, self.state, self.isServer, self.packetsToParse, whatever);
    };
    attempt();
  });
